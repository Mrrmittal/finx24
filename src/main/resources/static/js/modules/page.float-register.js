...
Router.register('float-register', function(panel) {
  ...
  async function _loadDashboard() {
    ...
    try {
      ...
      // Opening balance = MAX(balance) of the last month BEFORE period starts
      var openingBal = 0;
      if(fromLbl) {
        ...
        if(prevMonths.length) {
          openingBal = Number(prevMonths[prevMonths.length-1].closingBal||0);
        }
      }
      ...
      var closBal   = rows.length ? Number(rows[rows.length-1].closingBal||0)
          : (summ.length ? Number(summ[summ.length-1].closingBal||0) : 0);
      ...
      if(summ.length){
        ...
        // Opening bal for first row in table = last row before it
        summ.forEach(function(r,idx){
          var ob  = idx>0 ? Number(summ[idx-1].closingBal||0) : 0;
          var cb  = Number(r.closingBal||0);
          var exp = Number(r.totalDebit||0) - Number(r.cancelCredit||0);
          var hl  = inRange(r.monthLabel) ? 'background:#FFF8E7;' : 'opacity:0.55;';
          tbl+='<tr style="'+hl+'">'
              +'<td style="font-weight:500">'+r.monthLabel+'</td>'
              +'<td style="text-align:right">'+INR(ob)+'</td>'
              +'<td style="text-align:right;color:#1D6F42">'+INR(r.topUpCredit)+'</td>'
              +'<td style="text-align:right;color:#1E3F6B">'+INR(r.cancelCredit)+'</td>'
              +'<td style="text-align:right;color:#5A2D82">'+INR(r.totalDebit)+'</td>'

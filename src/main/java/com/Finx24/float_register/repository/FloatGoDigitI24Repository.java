...
public interface FloatGoDigitI24Repository extends GenericFloatRepository<FloatGoDigitI24> {
    ...
    @Query("SELECT r.monthLabel, " +
           ...
           "FROM FloatGoDigitI24 r GROUP BY r.monthLabel ORDER BY r.monthLabel ASC")
    ...
    @Query("SELECT r.periodLabel, " +
           ...
           "         THEN r.creditAmt ELSE 0 END), " +
           "SUM(r.debitAmt), MAX(r.balance) " +
           "FROM FloatGoDigitI24 r GROUP BY r.periodLabel ORDER BY r.periodLabel ASC")
    List<Object[]> findPeriodSummary();

    /** Register rows for a specific month */
    List<FloatGoDigitI24> findByMonthLabelOrderByTransDateAsc(String monthLabel);
}

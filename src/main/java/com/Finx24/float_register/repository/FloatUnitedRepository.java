...
public interface FloatUnitedRepository extends GenericFloatRepository<FloatUnited> {
    ...
    @Query("SELECT r.monthLabel, " +
           ...
           "FROM FloatUnited r GROUP BY r.monthLabel ORDER BY r.monthLabel ASC")
    ...
    @Query("SELECT r.periodLabel, " +
           ...
           "         THEN r.creditAmt ELSE 0 END), " +
           "SUM(r.debitAmt), MAX(r.balance) " +
           "FROM FloatUnited r GROUP BY r.periodLabel ORDER BY r.periodLabel ASC")
    List<Object[]> findPeriodSummary();

    /** Register rows for a specific month */
    List<FloatUnited> findByMonthLabelOrderByTransDateAsc(String monthLabel);
}

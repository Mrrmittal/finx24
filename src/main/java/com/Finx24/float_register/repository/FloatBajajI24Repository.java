...
public interface FloatBajajI24Repository extends GenericFloatRepository<FloatBajajI24> {
    ...
    @Query("SELECT DISTINCT r.monthLabel FROM FloatBajajI24 r ORDER BY r.monthLabel ASC")
    List<String> findAvailableMonths();

    @Query("SELECT DISTINCT r.periodLabel FROM FloatBajajI24 r ORDER BY r.periodLabel ASC")
    ...
    @Query("SELECT r.monthLabel, " +
           ...
           "FROM FloatBajajI24 r GROUP BY r.monthLabel ORDER BY r.monthLabel ASC")
    ...
    @Query("SELECT r.periodLabel, " +
           ...
           "         THEN r.creditAmt ELSE 0 END), " +
           "SUM(r.debitAmt), MAX(r.balance) " +
           "FROM FloatBajajI24 r GROUP BY r.periodLabel ORDER BY r.periodLabel ASC")
    List<Object[]> findPeriodSummary();

    /** Register rows for a specific month */
    List<FloatBajajI24> findByMonthLabelOrderByTransDateAsc(String monthLabel);
}

...
public interface FloatIciciGSRepository extends GenericFloatRepository<FloatIciciGS> {
    ...
    @Query("SELECT r.monthLabel, " +
           ...
           "FROM FloatIciciGS r GROUP BY r.monthLabel ORDER BY r.monthLabel ASC")
    ...
    @Query("SELECT r.periodLabel, " +
           ...
           "         THEN r.creditAmt ELSE 0 END), " +
           "SUM(r.debitAmt), MAX(r.balance) " +
           "FROM FloatIciciGS r GROUP BY r.periodLabel ORDER BY r.periodLabel ASC")
    List<Object[]> findPeriodSummary();

    /** Register rows for a specific month */
    List<FloatIciciGS> findByMonthLabelOrderByTransDateAsc(String monthLabel);
}

...
@Repository
public interface FloatGoDigitLIRepository extends GenericFloatRepository<FloatGoDigitLI> {

    @Query("SELECT DISTINCT r.monthLabel FROM FloatGoDigitLI r ORDER BY r.monthLabel ASC")
    List<String> findAvailableMonths();

    @Query("SELECT DISTINCT r.periodLabel FROM FloatGoDigitLI r ORDER BY r.periodLabel ASC")
    ...
    @Query("SELECT r.monthLabel, " +
           ...
           "FROM FloatGoDigitLI r GROUP BY r.monthLabel ORDER BY r.monthLabel ASC")
    ...
    @Query("SELECT r.periodLabel, " +
           ...
           "         THEN r.creditAmt ELSE 0 END), " +
           "SUM(r.debitAmt), MAX(r.balance) " +
           "FROM FloatGoDigitLI r GROUP BY r.periodLabel ORDER BY r.periodLabel ASC")
    List<Object[]> findPeriodSummary();

    /** Register rows for a specific month */
    List<FloatGoDigitLI> findByMonthLabelOrderByTransDateAsc(String monthLabel);
}
